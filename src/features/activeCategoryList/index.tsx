...
export const ActiveCategoryList = ({ inCategoryModal = false }) => {
  ...
  if (isFilteredByGender) data.push(isGenderMale ? "Для нього" : "Для неї");
  if (isFilteredByAge) data.push(`до ${restrictionAge}`);
  if (inCategoryModal) if (!data.length) return null;
  return (
    <View
      style={{
        width: windowWidth,
