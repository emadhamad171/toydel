  ...
  return (
    ...
    >
      ...
      <View style={{ flexDirection: "row", gap: 15, marginRight: 10 }}>
        ...
        <TouchableOpacity
          onPress={() => navigation.navigate({ name: "Cart" } as never)}
        >
    ...
    >
      ...
      >
        <Input
          ...
          onChangeAction={(text: string) => {
            setSearch(() => text);
          }}
          value={searchString}
          style={{
            backgroundColor: "#c9aaff",
            borderRadius: 10,
            maxWidth: 330,
            width: "100%",
            padding: 10,
...
const CategoryChipItem = ({ name, isSelected, onPressAction }) => {
  return (
    <TouchableOpacity onPress={() => onPressAction(isSelected)}>
...
const FilterModal = ({ setModal, setSelectedCategory, selectedCategories }) => {
  ...
  const addCategory = (name: string) => {
    setCategories((prevState) => [...prevState, name]);
  };
  ...
  return (
    <View>
      <CategoryChipItem
        name={"Fun"}
        isSelected={categories.includes("Fun")}
        onPressAction={(isSelected: boolean) => {
          isSelected ? removeCategory("Fun") : addCategory("Fun");
        }}
      />
      <CategoryChipItem
        name={"Games"}
        isSelected={categories.includes("Games")}
        onPressAction={(isSelected: boolean) => {
          isSelected ? removeCategory("Games") : addCategory("Games");
        }}
      />
      <TouchableOpacity
        style={{
...
const searchByString = ({ fetchedListItems, searchString }) => {
  return fetchedListItems.filter((item) => {
    return (
      item &&
      item.name &&
            ...
            setModal(() => {
              return () => (
                <FilterModal
                  ...
                  setSelectedCategory={setChoosedCategory}
                />
              );
            });
            setModalName("Filter");
...
const PageButtonsComponent = ({
  ...
  >
    ...
    >
      <Text style={{ fontSize: 16 }}>Previous Page</Text>
    </TouchableOpacity>
    <View style={{ flexDirection: "row", gap: 4 }}>
      <Text style={{ color: "#777" }}>
        {pageNumber - 1 ? pageNumber - 1 : ""}
      </Text>
      <Text style={{ fontSize: 16 }}>{pageNumber}</Text>
      <Text style={{ color: "#777" }}>
    ...
    >
      <Text style={{ fontSize: 16 }}>Next Page</Text>
    </TouchableOpacity>
  </View>
);
const Home = () => {
  ...
  const [fetchedListItems, setFetchedListItems] =
    useState<itemType[]>(itemsStackSample);
  const [isRefreshing, setRefreshing] = useState<boolean>(true);
  const [displayedListItems, setDisplayedListItems] =
    useState<itemType[]>(itemsStackSample);
  ...
  const [searchedListItems, setSearchedItems] =
    useState<itemType[]>(itemsStackSample);
  ...
  useEffect(() => {
    const searchedItems = searchByString({
      fetchedListItems,
      searchString,
    }).filter((item) => {
      if (
    ...
    setDisplayedListItems(
      getDisplayedList({ fetchedListItems: searchedItems, page: pageNumber }),
    );
  }, [searchString, selectedCategories, fetchedListItems, pageNumber]);

  useEffect(() => {
    loadData({
      setListItems: setFetchedListItems,
      setRefreshing,
      setFavoriteList,
      userID: user.id,
    });
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <ItemComponent
                ...
                setFavoriteToyList={setFavoriteList}
                isFavorite={favoriteList && favoriteList.includes(item.id)}
                onClickMore={itemOnClick({ item })}
              />
            )}
            ListHeaderComponent={() => (
            ...
            ListFooterComponent={() => (
              <PageButtonsComponent
                ...
                searchedListItems={searchedListItems}
              />
            )}
          />
        </View>
